        ...
        {!isDataSubmitted && (
          <>
            ...
            <input onChange={(e) => setPassword(e.target.value)} value={password} type="password" placeholder="Password" required
              className="p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500" minLength={6} />
          </>
        )}

        {
          currState === "Sign up" && isDataSubmitted && (
            <textarea onChange={(e) => setBio(e.target.value)} value={bio} rows={4} placeholder="provide a short bio..." required
              className="p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"></textarea>
          )
        }
